...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/mlst/usage](https://nf-co.re/mlst/usage)
...
```console
nextflow run nf-core/mlst --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/mlst releases page](https://github.com/nf-core/mlst/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
